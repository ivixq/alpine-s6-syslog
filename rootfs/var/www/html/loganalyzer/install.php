...
$content['HeaderDefaultEncoding'] = "UTF-8"; // Set Default encoding!  
...
{	
	...
	if ( isset($_SESSION['UserDBServer']) ) { $content['UserDBServer'] = $_SESSION['UserDBServer']; } else { $content['UserDBServer'] = "<DB_HOST>"; }
	if ( isset($_SESSION['UserDBPort']) ) { $content['UserDBPort'] = $_SESSION['UserDBPort']; } else { $content['UserDBPort'] = "3306"; }
	if ( isset($_SESSION['UserDBName']) ) { $content['UserDBName'] = $_SESSION['UserDBName']; } else { $content['UserDBName'] = "<DB_NAME>"; }
	if ( isset($_SESSION['UserDBPref']) ) { $content['UserDBPref'] = $_SESSION['UserDBPref']; } else { $content['UserDBPref'] = "logcon_"; }
	if ( isset($_SESSION['UserDBUser']) ) { $content['UserDBUser'] = $_SESSION['UserDBUser']; } else { $content['UserDBUser'] = "<DB_USER>"; }
	if ( isset($_SESSION['UserDBPass']) ) { $content['UserDBPass'] = $_SESSION['UserDBPass']; } else { $content['UserDBPass'] = "<DB_PASS>"; }
	...
	if ( $_SESSION['UserDBEnabled'] == 1 )
	{
		if ( $_SESSION['UserDBAuthMode']  == USERDB_AUTH_INTERNAL )
		{
			if ( isset($_SESSION['MAIN_Username']) )
				...
				$content['MAIN_Username'] = "<SYSLOG_ADMIN>";

			$content['MAIN_Password1'] = "<ADMIN_PASS>";
			$content['MAIN_Password2'] = "<ADMIN_PASS>";
...
else if ( $content['INSTALL_STEP'] == 7 )
{
	...
	if ( isset($_SESSION['SourceDBName']) ) { $content['SourceDBName'] = $_SESSION['SourceDBName']; } else { $content['SourceDBName'] = "<DB_NAME>"; }
	if ( isset($_SESSION['SourceDBServer']) ) { $content['SourceDBServer'] = $_SESSION['SourceDBServer']; } else { $content['SourceDBServer'] = "<DB_HOST>"; }
	...
	if ( isset($_SESSION['SourceDBUser']) ) { $content['SourceDBUser'] = $_SESSION['SourceDBUser']; } else { $content['SourceDBUser'] = "<DB_USER>"; }
	if ( isset($_SESSION['SourceDBPassword']) ) { $content['SourceDBPassword'] = $_SESSION['SourceDBPassword']; } else { $content['SourceDBPassword'] = "<DB_PASS>"; }
